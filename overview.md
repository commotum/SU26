## MTH-252: Integral Calculus

- **Exams: 53% total**
  - **Mini-Exams 1-4:** 4 exams, **40% total**, 10% each.
  - **Mini-Exam 5:** 1 exam, **13%**.
- **Homework: 40% total**
  - **Achieve-Online HW:** ~2/week, 18 assignments, **1.12%** each.
  - **WHW / Written Homework:** ~1/week, 10 assignments, **2%** each.
- **Participation: 7% total**
  - **Discussion Reflection (DR):** ~1/week, 9 discussions, **0.45%** each.
  - **Reading Quizzes (RQ):** ~1-2/week, 15 quizzes, **0.2%** each.


## MTH-253: Sequences and Series

- **Exams: 55% total**
  - **Unit Quizzes:** 4 quizzes, **40% total**.
  - **Final Exam:** 1 exam, **15%**.
- **Homework: 35% total**
  - **Online Homework / Achieve Web Homework:** ~1/week, 10 weekly sets, **2%** each.
  - **Written Homework:** ~1/unit, 5 unit assignments, **3%** each.
- **Participation: 10% total**
  - **Discussion Boards:** ~3/term, 3 graded discussions, **3.34%** each.


## PHY-212: General Physics with Calculus

- **Exams: 55% total**
  - **Quizzes:** 3 quizzes, **40% total**, lowest quiz dropped.
  - **Final Exam:** 1 cumulative final, **15%**.
- **Homework: 10% total**
  - **Homework:** ~1-2/week, 10 scheduled homework sets, **1%** each, lowest homework dropped.
- **Labs: 25% total**
  - **Lab discussions/prep:** ~1/week, 8 discussions, **1.05%** each.
  - **Lab reports:** ~1/week, 8 reports, **2.09%** each.
  - Must pass lab to pass the course.
- **Participation: 10% total**
  - **Formative Assessment / Participation:** ~3-4/week, ~28 participation items, **0.18%** each; three lowest dropped.
  - **Pre-lecture Questions:** ~3-4/week, ~28 `pre` assignments, **0.18%** each; three lowest dropped.


## Weekly Load

- **Exams:** usually 1 major exam/quiz most weeks; some weeks have 2-3 stacked.
  - **MTH-252:** mini-exam about every other week.
  - **MTH-253:** unit quiz about every other week.
  - **PHY-212:** quiz about every other week, plus final.
- **Homework:** ~5-7 items/week.
  - **MTH-252:** ~2 Achieve items and 1 WHW/week.
  - **MTH-253:** ~1 Achieve item/week, plus written homework by unit.
  - **PHY-212:** ~1-2 homework sets/week.
- **Labs:** ~1.6-2 items/week.
  - **PHY-212:** ~1 lab discussion/prep and 1 lab report/week, plus TA/group meeting.
- **Participation:** ~10-11 items/week.
  - **PHY-212:** ~7-8 small quizzes/week.
  - **MTH-252:** ~1 discussion and ~2 quizzes/week.
  - **MTH-253:** occasional discussion board.


## Exam Schedule

- **Week 1** (6/22-6/28)
  - **No Tests**.

- **Week 2** (6/29-7/5)
  - **MTH-253:** Unit 1 Quiz, Tue 6/30, **10%**.

- **Week 3** (7/6-7/12)
  - **PHY-212:** Quiz 1, Mon 7/6, **20% counted if retained**.
  - **MTH-252:** Mini-Exam 1, Thu 7/9, **10%**.

- **Week 4** (7/13-7/19)
  - **MTH-253:** Unit 2 Quiz, Mon 7/13, **10%**.

- **Week 5** (7/20-7/26)
  - **PHY-212:** Quiz 2, Mon 7/20, **20% counted if retained**.
  - **MTH-252:** Mini-Exam 2, Thu 7/23, **10%**.

- **Week 6** (7/27-8/2)
  - **No Tests**.

- **Week 7** (8/3-8/9)
  - **MTH-253:** Unit 3 Quiz, Mon 8/3, **10%**.
  - **PHY-212:** Quiz 3, Mon 8/3, **20% counted if retained**.
  - **MTH-252:** Mini-Exam 3, Thu 8/6, **10%**.

- **Week 8** (8/10-8/16)
  - **PHY-212:** Final Exam, Thu 8/13, **15%**.

- **Week 9** (8/17-8/23)
  - **MTH-252:** Mini-Exam 4, Thu 8/20, **10%**.

- **Week 10** (8/24-8/30)
  - **MTH-253:** Unit 4 Quiz, Mon 8/24, **10%**.
  
- **Week 11** (8/31-9/6)
  - **MTH-253:** Final Exam, Tue 9/1, **15%**.
  - **MTH-252:** Final Mini-Exam 5, Wed 9/2, **13%**.
  - **MTH-252:** Mini-Exam Retakes, Wed 9/2, **optional replacement**.
